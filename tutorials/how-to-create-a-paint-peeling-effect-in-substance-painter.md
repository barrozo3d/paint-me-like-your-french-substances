...
- [HOW to Make a Peeled Paint Effect with ANCHOR Points | SUBSTANCE PAINTER](how-to-make-a-peeled-paint-effect-with-anchor-points-substance-painter.md) — same creator (Jared Chavez) as the anchor-points tutorial above; a fourth independent take on the same peeling-paint effect family, isolating the lifted edge via a double-anchor-reference with `Subtract` blend mode.
- [Anchor Point Magic 03 - Paint Peel Effect in Substance 3D Painter](anchor-point-magic-03---paint-peel-effect-in-substance-3d-painter-adobe-substanc.md) — Adobe official; a more minimal from-scratch peeling-paint build (double anchor reference, inverted + Multiply-clipped, plus a Blur filter) rather than this video's Mask Outline filter and filter-native blend modes — good side-by-side at a different complexity level.
