...
- [Texturing Women's Shorts with Lace Trim in Substance Painter](texturing-womens-shorts-with-lace-trim-in-substance-painter.md) — same creator; the wrinkle layers' Height-channel-to-Linear-Dodge trick there is the same channel-specific blend-mode approach used for this stool's border-effect layer.
- [How to Create a Realistic Poison Bottles Material Using Substance Painter](how-to-create-a-realistic-poison-bottles-material-using-substance-painter.md) — same creator; shares the Pass-Through "collector" layer pattern (Ornament Plus there, Edge Plus here) used to feed height/AO data into downstream generators via anchor points.
...
- [How to use UDIMs properly!](how-to-use-udims-properly.md) — same creator; both address texel-density problems on many-small-pieces models (overlapping UVs there as a shortcut, texel-density-locked UDIM packing here as the rigorous method).
