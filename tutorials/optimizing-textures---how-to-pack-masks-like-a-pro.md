...
- [SUBSURFACE SCATTERING: Subsurface Scattering in SUBSTANCE PAINTER for UNREAL ENGINE 5](subsurface-scattering-subsurface-scattering-in-substance-painter-for-unreal-engi.md) — different creator (Jared Chavez); shares the "author a custom Texture Set channel in Painter, then reconstruct/consume it via a dedicated Unreal asset" pattern (there: a "Scattering" channel driving a SubsurfaceProfile asset; here: a packed "blood" channel driving a Material Instance parameter).
