...
- **HOW to Make a Peeled Paint Effect with ANCHOR Points | SUBSTANCE PAINTER** (`tutorials/how-to-make-a-peeled-paint-effect-with-anchor-points-substance-painter.md`) — different creator (Jared Chavez), same peeling-paint goal via a very similar double-anchor-reference technique (here: invert-then-Multiply-clip; there: grow-then-Subtract) — useful side-by-side comparison of two studios' takes on the same underlying trick.
