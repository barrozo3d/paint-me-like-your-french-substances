...
## Related Tutorials
None yet cross-linked — this is the first tutorial ingested into this skill's library. Future ingests covering anchor points (Jared Chavez's "Building Masks Explained," "Peeled Paint Effect with ANCHOR Points"), leather/fabric materials, decal/sticker workflows, PBR export-to-Unreal pipelines, or J Hill's companion skin-texturing video (`s0DhvFML7oM`) should cross-link back here — this video is the deepest single source in the library on anchor-point-driven multi-effect masking, the Bevel Smooth filter, and the Overlays composite pass.
