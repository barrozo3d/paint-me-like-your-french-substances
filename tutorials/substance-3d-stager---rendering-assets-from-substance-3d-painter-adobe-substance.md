...
- [Creating environment materials and meshes in Substance 3D Painter](creating-environment-materials-and-meshes-in-substance-3d-painter.md) — same channel (Adobe Substance 3D); uses the same Send-to-Stager handoff specifically to ray-trace-validate real height-displacement on a tileable environment mesh, including raising the default (far too low) tessellation face budget for detailed displacement.
