...
- [HOW to Make UDIMS for UNREAL ENGINE](how-to-make-udims-for-unreal-engine.md) (Jared Chavez) — different creator; another UDIM-focused video with a similarly thin direct-Painter-content share (there: mandatory `Use UV Tile` import toggle + Thickness-seam fix; here: RizomUV overlap-fixing + a Texel Density generator check) — useful comparison of two independent UDIM pipelines around Painter.
...
- [Using UV set and Stencils In Substance Painter -- English version](using-uv-set-and-stencils-in-substance-painter----english-version.md) — same creator; shares the exact same "second UV channel authored purely to fix per-island scale/rotation, referenced via a Fill layer's UV set to UV set / secondary-UV-set projection source" technique this video verifies at the generator-heatmap level.
- More 3DRedBox tutorials (NavyCap) will be cross-linked here as they are ingested — see `tutorials/INDEX.md` for the current full list.
