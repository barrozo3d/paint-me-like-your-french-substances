...
## Related Tutorials
- [How to Make the Substance Painter Viewport Match Unreal Engine](how-to-make-the-substance-painter-viewport-match-unreal-engine.md) — same topic, alternate creator (Quinn Kuslich), same underlying Brian Leleux ACES LUT pack and Tone-Mapping-Log-then-Activate-Color-Profile sequence; that video adds the Environment Opacity/background-matching step and frames the fix around Roughness/Metallic accuracy specifically, while this video adds the rigorous Iray-vs-UE4-Path-Tracing validation pass. Treat as a matched pair.
