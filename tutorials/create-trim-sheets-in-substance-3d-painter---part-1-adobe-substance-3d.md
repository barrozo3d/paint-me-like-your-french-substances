...
## Related Tutorials
- **Create Trim Sheets in Substance 3D Painter - Part 2** (`tutorials/create-trim-sheets-in-substance-3d-painter---part2-adobe-substance-3d.md`) — direct continuation: base color pass for this same trim sheet, plus using the finished result inside Unreal Engine 5.
...
- **Footwear Texturing from Start to Finish – Live Tutorial in Substance 3D Painter** (`tutorials/footwear-texturing-from-start-to-finish-live-tutorial-in-substance-3d-painter-ad.md`, video `s59xbaF4Q14`) — shares the same transcript-quality caveat (Whisper mis-detected the language for large stretches); Structured Notes for both were reconstructed the same way, from surviving clear terms cross-checked against frames.
