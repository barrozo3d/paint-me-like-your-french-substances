...
- Several more Path/Ribbon-tool videos are planned later in this same ingest batch (`rhraMw3YVpo` "3D Path Tool Updates", `Ro5dADu3vpM` "Creating 3D Paths") — cross-link those reciprocally once ingested, since they'll likely build directly on the Ribbon fundamentals covered here.
