...
- [How to create a paint peeling effect in Substance Painter](how-to-create-a-paint-peeling-effect-in-substance-painter.md) — different creator (Wes McDermott), same core subject (procedural paint peeling) and the same anchor-point-as-shared-mask-source pattern; that video leans on the Mask Outline filter and filter-native blend modes where this one leans on Voronoi-fractal noise and a Multiply-Light paint-reveal trick — strong side-by-side comparison for the same effect family.
- [HOW to Make a Peeled Paint Effect with ANCHOR Points | SUBSTANCE PAINTER](how-to-make-a-peeled-paint-effect-with-anchor-points-substance-painter.md) — different creator (Jared Chavez), same effect family; that video isolates the lifted edge ring via a double-anchor-reference with `Subtract` blend mode (grow-then-cancel) versus this video's Voronoi-fractal/Lighten-Divide mask math — a third independent take on the same core problem.
- [10 New Features in Substance Painter You Didn't Know About](10-new-features-in-substance-painter-you-didnt-know-about.md) — different creator (FlippedNormals); this video's UDIM tile projection setup is a direct real-world use case for that feature-tour video's UDIM/UV Tiles support entry.
- [Anchor Point Magic 03 - Paint Peel Effect in Substance 3D Painter](anchor-point-magic-03---paint-peel-effect-in-substance-3d-painter-adobe-substanc.md) — Adobe official; a more minimal from-scratch peeling-paint build using a double anchor reference (inverted + Multiply-clipped) instead of this video's Voronoi/Lighten-Divide mask math — good side-by-side for seeing the same core idea at two different complexity levels.
