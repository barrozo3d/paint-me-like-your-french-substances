#!/usr/bin/env python3
"""
validate.py -- Post-ingest integrity checker for paint-me-like-your-french-substances tutorials.
...
Checks performed:
  ...
  3. No app: "[PENDING]" or version: "[PENDING]" in frontmatter
...
def check_tutorials():
    ...
    for fname in files:
        ...
        if re.search(r'^app:\s*["\']?\[?PENDING', content, re.IGNORECASE | re.MULTILINE):
            fail(f"{fname}: app is still a PENDING placeholder")

        if re.search(r'^version:\s*["\']?\[?PENDING', content, re.IGNORECASE | re.MULTILINE):
            fail(f"{fname}: version is still a PENDING placeholder")
...
def main():
    ...
    print("=" * 60)
    print("paint-me-like-your-french-substances validate.py")
    print("=" * 60)
...
# ingest pipeline. Copies have historically drifted (missing UTF-8 fix, mismatched
# cookies flags), so when sibling skill dirs are present on this machine, compare
# the shared helper functions and WARN on differences. Never fails the run —
# per-skill divergence may be intentional, but it should always be a conscious choice.
...
SHARED_FUNCS = ("slugify", "download_audio", "ytdlp_captions", "segment_by_chapters",
                ...
                "fetch_article",
                # added 2026-09-03 (plan §3.7 items 1-2): the caption
                # cross-check and per-video prompt priming. Both carry a limit
                # that is the whole point and is easy to lose in a copy --
                # caption_crosscheck_note() must keep saying disagreement means
                # "look here" and never "Whisper is wrong", and
                # build_video_prompt() must keep DROPPING non-Latin metadata.
                # A drifted copy that quietly lost either would look like it
...
WATCHED_FILES = {
    ...
    # loosens the cutoff or widens the affix suppression turns a 76%-precision
